...
gridButton.addEventListener("click", async (e) => {
  ...
  const sectionArray = await gridCardBuilder(data,true);
...
listButton.addEventListener("click", async (e) => {
  ...
  memberContainer.innerHTML = "";
  const data = await memberList();
  const TableList = await ListCardBuilder(data);
  memberContainer.appendChild(TableList);
});

function ListCardBuilder(data) {
  let level = "";
...
const Start = async () => {
  const data = await memberList();
  // console.log(data);
  const arraySection = await gridCardBuilder(data,true);
