...
function showList(list) {
  const allList = list.reduce((total, item) => {
    return (
      total +
      `<li>${item.completed ? "<strong>✓</strong> " : ""}${item.subject} ${item.number}${item.completed ? "" : " - <span>in progress</span>"}</li>`
    );
  }, "");
...
filter.addEventListener("submit", (e) => {
  ...
  const CreditTotal = filteredList.reduce((total, item) => {
    return (total += item.credits);
  }, 0);

  coursesInfo.innerHTML = `The total credits for course listed above <strong>${CreditTotal}</strong>.`;
});
